document.addEventListener("DOMContentLoaded", () => {
    ...
    function checkBoard() {
        for (let y = 0; y < winArrays.length; y++) {
            ...
            const square4 = squares[winArrays[y][3]];

            // Check the win square for palyer-1
            if (square1.classList.contains('player-one') &&
                square2.classList.contains('player-one') &&
                square3.classList.contains('player-one') &&
                square4.classList.contains('player-one')) {
                result.innerHTML = "Player One 🐽 wins ! so player 2 🔵 rip🪦";
            }
            if (square1.classList.contains('player-two') &&
                square2.classList.contains('player-two') &&
                square3.classList.contains('player-two') &&
                square4.classList.contains('player-two')) {
                result.innerHTML = "Player Two 🔵 wins ! so player 2 🐽 rip🪦";
            }
        }
    }
